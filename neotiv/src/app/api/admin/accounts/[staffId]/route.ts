...
export async function PATCH(request: Request, context: { params: Promise<{ staffId: string }> }) {
  try {
    ...
    const { action, is_active, role, hotel_id } = body;
    ...
    const { data: staffData } = await adminSupabase.from('staff').select('user_id, email, hotel_id').eq('id', staffId).single();
    ...
    if (action === 'suspend') {
        ...
        if (userData.user) {
          const newMetadata = { ...userData.user.user_metadata, role, hotel_id };
          await adminSupabase.auth.admin.updateUserById(staffData.user_id, { user_metadata: newMetadata });
        }
        await adminSupabase.from('staff').update({ role, hotel_id }).eq('id', staffId);
    }
    ...
    await adminSupabase.from('activity_log').insert({
      ...
      action: action === 'suspend' ? (is_active ? 'staff.reactivated' : 'staff.suspended') : 'staff.role_updated',
      target_type: 'staff',
      target_id: staffId
    });
